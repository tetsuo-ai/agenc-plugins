...
test("Forge rejects invalid input, inherited styles and missing comparison source", () => {
...
test("Forge never passes blocking errors", () => {
  const result = verifyCode('console.log("debug");', { style: "defensivo" });
  assert.equal(result.score, 88); assert.equal(result.pass, false);
});
test("Forge does not suggest const for updated variables or remove used import aliases", () => {
...
test("Forge ignores debug-looking strings/comments and braces inside JS literals", () => {
